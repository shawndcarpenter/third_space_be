...
RSpec.describe ThirdSpace do
  ...
  it "can make a third space" do
    search = ThirdSpacePoro.new(@attrs)
    search.make_third_space
    space = ThirdSpace.last
    
    expect(space.address).to eq(search.address)
    expect(space.category).to eq(search.category)
    expect(space.name).to eq(search.name)
  end

  it "can choose an alternate image" do
    attrs = {
        "id": "f-m7-hyFzkf0HSEeQ2s-9A",
        "alias": "fog-harbor-fish-house-san-francisco-2",
        "name": "Fog Harbor Fish House",
    ...
    expect(search.photos).to eq("https://s3-media2.fl.yelpcdn.com/bphoto/by8Hh63BLPv_HUqRUdsp_w/o.jpg")
  end
end